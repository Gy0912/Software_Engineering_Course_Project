...
class TestElevatorSystemIntegration(unittest.TestCase):
    ...
    def test_05_multiple_concurrent_requests(self):
        """Multiple requests"""
        ...
        # 等待电梯移动完成
        #  QTest.qWait(9000)
        while (self.system.elevators[0].state == ElevatorState.up or self.system.elevators[1].state == ElevatorState.down):
            QTest.qWait(50)

        self.assertEqual(self.system.elevators[0].current_floor, 3)
        self.assertEqual(self.system.elevators[1].current_floor, 0)
        while (self.system.elevators[0].state != ElevatorState.stopped_door_closed or self.system.elevators[1].state != ElevatorState.stopped_door_closed):
            QTest.qWait(50)
