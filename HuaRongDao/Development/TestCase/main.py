...
TESTCASE_ID = 8
test_file = f"./Development/TestCase/testcase/testcase.txt"
answer_file = f"./Development/TestCase/answer/answer.txt"
    ...
    while(True):
        if (is_received_new_message(timeStamp, clientMessage)):
            ...
            if not clientMessage.startswith("Client"):
                correct_count += (clientMessage == answer_list[answer_count])
                answer_count += 1

            if answer_count == len(answer_list):
                result = "PASSED!" if answer_count == correct_count else "FAILED!"
