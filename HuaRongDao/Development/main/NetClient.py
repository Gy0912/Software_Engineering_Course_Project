...
class ZmqClientThread(threading.Thread):
    ...
    def __init__(self, serverIp="127.0.0.1", port="27132", identity="GroupX"):
        ...
        self._port = port
        self._socket.setsockopt_string(zmq.IDENTITY,
                                       identity)  # default encoding is UTF-8 #Set your IDENTITY before connection.
        ...
        self._socket.connect("tcp://{0}:{1}".format(serverIp,
                                                    port))  # Both ("tcp://localhost:27132") and ("tcp://127.0.0.1:27132") are OK
    ...
    def __launch(self, socket):
        while True:
            if not socket.closed:
                ...
                print(
                    "Message from server: " + message_str)  # Helpful for debugging. You can comment out this statement.
