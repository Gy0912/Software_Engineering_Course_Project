...
class HuarongDaoGame:
    ...
    def set_board(self, board_str):
        if len(board_str) != 20:
            return False, 'Invalid board string length' #TC1/2
        for ch in board_str:
            if ch != '#' and ch not in self.piece_sizes: 
                return False, f'Invalid piece id: {ch}' #TC3/4
        self.board = list(board_str)
        self.history.clear()
        self.steps = 0
        self.start_time = time.time()
        # 判断是否是已知关卡
        self.current_level = next((name for name, s in self.levels.items() if s == board_str), None) #TC5/6
    ...
    def move_piece(self, piece, direction):
        if piece is None: #TC1/2
            return False, 'Invalid move!' 
        if piece not in self.board: #TC3/4
            return False, 'Invalid move!'
        w, h = self.piece_sizes.get(piece, (1, 1))
        positions = [i for i, p in enumerate(self.board) if p == piece] #TC5/6
        rows = [pos // 4 for pos in positions]
        cols = [pos % 4 for pos in positions]
        min_r, min_c = min(rows), min(cols)
        dr, dc = 0, 0
        if direction == 'Up': dr = -1 #TC7/8
        elif direction == 'Down': dr = 1 #TC9/10
        elif direction == 'Left': dc = -1 #TC11/12
        elif direction == 'Right': dc = 1 #TC13/14
        else: return False, 'Invalid move!' 
        new_r = min_r + dr
        new_c = min_c + dc
        if new_r < 0 or new_c < 0 or new_c + w > 4 or new_r + h > 5: #TC15/16
            return False, 'Invalid move!'
        if dr == -1: #TC17/18
            for c in range(min_c, min_c + w):
                if self.board[(min_r - 1) * 4 + c] not in ('#', piece): return False, 'Invalid move!' #TC25/26
        elif dr == 1: #TC19/20
            for c in range(min_c, min_c + w):
                if self.board[(min_r + h) * 4 + c] not in ('#', piece): return False, 'Invalid move!' #TC27/28
        elif dc == -1: #TC21/22
            for r in range(min_r, min_r + h):
                if self.board[r * 4 + (min_c - 1)] not in ('#', piece): return False, 'Invalid move!' #TC29/30
        elif dc == 1: #TC23/24
            for r in range(min_r, min_r + h):
                if self.board[r * 4 + (min_c + w)] not in ('#', piece): return False, 'Invalid move!' #TC31/32
    ...
    def get_state(self):
        elapsed = int(time.time() - self.start_time) if self.start_time else 0 #TC1/2
...
class ZmqFrontendServer(threading.Thread):
    ...
    def _send_state(self, msg: str = ""):
        ...
        elapsed = int(time.time() - self.game.start_time) if self.game.start_time else 0 #TC1/2
        if self.game.current_level == None: #TC3/4
...
# python Development\main\Unittest_backend.py
# python Development\main\Unittest_frontend.py
# python Development\main\IntegrateTest.py
# python Development\main\UIControl.py
# python Development\main\UIControl_api.py