...
class GameWindow(QWidget):
    ...
    def reset_game(self):
        if self.current_level and self.current_level != "Random": #TC1/2
            self.set_level(self.current_level)
        elif self.current_level == "Random": #TC3/4
            self.set_random_level()
    ...
    def on_response(self, msg):
        ...
        # LEVELS@l1,l2,...
        if msg.startswith("LEVELS@"): #TC5/6
            self.levels_list = msg.split("@",1)[1].split(",")
            return

        # HINT@<dir><pid>
        if msg.startswith("HINT@"): #TC7/8
